...
const Toolbar = ({ date, view, onClickNext, onClickPrev, onClickToday, onChangeView, viewOptions, ...others }) => {
    ...
    useEffect(() => {
        ...
        if (matchSm) {
            newOption = viewOptions.filter((options) => options.value !== 'dayGridMonth' && options.value !== 'timeGridDay');
        }
        setNewViewOption(newOption);
    }, [matchSm]);

    return (
        <Grid alignItems="center" direction="row-reverse" container justifyContent="space-between" spacing={3} {...others} sx={{ pb: 3 }}>
            <Grid item>
