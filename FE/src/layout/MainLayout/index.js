...
import { RolesActions } from 'store/slices/roles';
...
const MainLayout = () => {
    const { ioInstance } = useSockets();
    const theme = useTheme();
    const { user } = useAuth();

    const matchDownMd = useMediaQuery(theme.breakpoints.down('md'));
    const dispatch = useDispatch();
    const { fetchCompanyData } = CompanyActions;
    const { fetchRoles } = RolesActions;
    const { fetchTemplates, fetchTemplateVariables } = TemplateActions;
    ...
    return (
        <Box sx={{ display: 'flex' }}>
            ...
            <Main theme={theme} open={drawerOpen} layout={layout}>
                <Container maxWidth={container ? 'lg' : false} {...(!container && { sx: { px: { xs: 0 } } })} sx={{ height: '100%' }}>
                    {/* breadcrumb */}
